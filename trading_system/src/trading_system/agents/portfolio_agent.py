"""PortfolioAgent: the single writer of trading state (cash, positions,
trade history, daily equity) — same "single writer" invariant as the
baseline, backed by SQLite for durability across restarts.
"""
...
class PortfolioStore:
    """Synchronous SQLite persistence layer. Kept separate from the async
    Agent wrapper so it's trivially unit-testable without an event loop."""
    ...
    def apply_execution(self, execution: ExecutionResult, stop_loss_price: float | None) -> float:
        """Applies a fill to cash/positions/trades. Returns realized pnl for this trade (0 for buys)."""
        cash = self.cash
...
class PortfolioAgent(Agent):
    ...
    @property
    def goal(self) -> str:
        return "Maintain the single source of truth for cash, positions, and trade history"

    async def run(self, execution: ExecutionResult, stop_loss_price: float | None = None) -> AgentReport:
        realized = await asyncio.to_thread(self.store.apply_execution, execution, stop_loss_price)
        return AgentReport(
            agent_id=self.agent_id, status="success",
            message=f"Applied {execution.action} {execution.quantity} {execution.symbol}",
