...
from trading_system.core.models import Action, ExecutionResult
...
@pytest.mark.asyncio
async def test_portfolio_agent_run_wraps_store(tmp_path):
    store = _mk_store(tmp_path)
    agent = PortfolioAgent(store)
    exe = ExecutionResult(
        symbol="AAPL", action=Action.BUY, quantity=1, requested_price=100, filled_price=100,
        commission=0, slippage=0, status="filled", order_id="X1",
    )
    report = await agent.run(exe, stop_loss_price=None)
