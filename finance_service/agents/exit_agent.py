...
class ExitAgent(Agent):
    ...
    def __init__(
        ...
        if self.config:
            ...
            self.partial_target_pct = self.config.get(
                "risk", "partial_target_pct", default=0.02
            )
            self.trail_stop_multiplier = self.config.get(
                "risk", "trail_stop_multiplier", default=1.5
            )
        else:
    ...
    async def run(
        ...
        **kwargs,
        ...
        logger.info(
            f"ExitAgent: Checking {len(positions)} position(s) for reactive exits..."
        )
        ...
        if perform_strategy_check and self.analysis_agent:
            logger.info(
                f"ExitAgent: Performing strategic re-analysis on {len(positions)} position(s)..."
            )
        ...
        logger.info(message)
        return AgentReport(
            agent_id=self.agent_id, status="success", message=message, payload=payload
        )

    async def _check_reactive_exits(
        ...
        for pos in positions:
            ...
            if current_price is None and self.data_agent:
                try:
                    ...
                    if (
                        quote_report.status == "success"
                        and "dataframe" in quote_report.payload
                    ):
                        import pandas as pd
    ...
    async def _check_strategic_degradation(
        self, positions: List[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
        """
        ...
        for pos in positions:
            ...
            try:
                ...
                analysis_report = await self.analysis_agent.run(
                    data_payload=data_report.payload, symbol=symbol
                )
                ...
                try:
                    should_sell, exit_rules = (
                        self.strategy_agent.rule_strategy.evaluate_exit(indicators)
                    )
                ...
                logger.error(
                    f"Error checking strategic degradation for {symbol}: {e}",
                    exc_info=True,
                )
                continue
