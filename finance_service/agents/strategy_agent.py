...
from finance_service.indicators.models import (
    IndicatorsSnapshot,
    SignalType,
    IndicatorResult,
)
...
class RuleStrategy:
    ...
    def __init__(self, rules_config: List[Dict]):
        ...
        logger.info(
            f"RuleStrategy initialized: {len(self.entry_rules)} entry, {len(self.exit_rules)} exit rules"
        )
    ...
    def evaluate_entry(
        self, indicators_snapshot: IndicatorsSnapshot
        ...
        for rule in self.entry_rules:
            ...
            if not ind:
                logger.warning(
                    f"Rule {rule.name}: indicator {rule.indicator} not found"
                )
                continue
        ...
        confidence = (
            len(triggered) / len(enabled_entry_rules) if enabled_entry_rules else 0.0
        )

        logger.info(
            f"Entry evaluation: {len(triggered)}/{len(enabled_entry_rules)} rules triggered (conf: {confidence:.2%})"
        )
    ...
    def evaluate_exit(
        ...
        if should_sell:
            logger.info(
                f"Exit evaluation: {len(triggered)} rules triggered: {triggered}"
            )
...
class StrategyAgent(Agent):
    ...
    def __init__(self, config_engine, portfolio_agent=None):
        ...
        self.initial_cash = self.config_engine.get(
            "finance", "portfolio/initial_cash", default=100000.0
        )
        ...
        self.position_cooling_hours = self.config_engine.get(
            "finance", "strategy/position_cooling_hours", default=24
        )
        logger.info(
            f"StrategyAgent: portfolio_value=${self.initial_cash:,.2f}, risk_budget_pct={self.risk_budget_pct}%, position_cooling_hours={self.position_cooling_hours}"
        )
    ...
    def _load_rules_from_config(self) -> List[Dict]:
        ...
        if strategy_name:
            ...
            if isinstance(strategies, dict) and strategy_name in strategies:
                ...
                logger.info(
                    f"Loaded strategy '{strategy_name}' with {len(entry_rules)} entry and {len(exit_rules)} exit rules"
                )
                return all_rules
            else:
                available = (
                ...
                logger.error(
                    f"Strategy '{strategy_name}' not found in strategies. Available: {available}"
                )
        ...
        if isinstance(rules_enabled, dict):
            rule_map = {
                "rsi_entry_oversold": {
                    ...
                    "indicator": "rsi",
                    "condition": "less_than",
                    "value": self.config_engine.get(
                        "finance",
                        "strategy/rules/rsi_entry_oversold_threshold",
                        default=35,
                    ),
                },
                "macd_crossover": {
                ...
                "rsi_exit_overbought": {
                    ...
                    "value": self.config_engine.get(
                        "finance",
                        "strategy/rules/rsi_exit_overbought_threshold",
                        default=70,
                    ),
                },
                "macd_signal_exit": {
    ...
    async def run(
        self, analysis_payload: Dict[str, Any], symbol: str = None
    ) -> AgentReport:
        """
        ...
        try:
            ...
            should_buy, confidence, entry_rules = self.rule_strategy.evaluate_entry(
                indicators_snapshot
            )
            should_sell, exit_rules = self.rule_strategy.evaluate_exit(
                indicators_snapshot
            )
            ...
            if should_buy:
                ...
                if last_entry:
                    ...
                    if (now - last_entry).total_seconds() < cooling_hours * 3600:
                        logger.info(
                            f"Position cooling: skipping {symbol} - last entry was {(now - last_entry).total_seconds() / 3600:.1f}h ago (minimum {cooling_hours}h)"
                        )
                        return AgentReport(
                            agent_id=self.agent_id,
                            status="success",
                ...
                if self.portfolio_agent:
                    try:
                        ...
                        portfolio_report = await self.portfolio_agent.run(
                            event_type=Events.GET_PORTFOLIO_STATE, payload={}
                        )
                        if portfolio_report.status == "success":
                            ...
                            for pos in portfolio_data.get("positions", []):
                                if pos["symbol"] == symbol:
                                    ...
                                    break
                            portfolio_equity = portfolio_data["equity_metrics"][
                                "total_equity"
                            ]
                            # get max position size from risk config (fallback)
                            max_position_size_pct = self.config_engine.get(
                                "finance", "risk/max_position_size_pct", default=10.0
                            )
                    except Exception as e:
                        logger.warning(
                            f"Failed to query portfolio_agent for position cooling: {e}"
                        )
                ...
                # Fetch dynamically from config
                atr_multiplier_sl = self.config_engine.get(
                    "finance", "risk/atr_multiplier_sl", default=2.0
                )
                atr_multiplier_tp = self.config_engine.get(
                    "finance", "risk/atr_multiplier_tp", default=4.0
                )

                stop_loss_price = round(
                    current_price - (atr_value * atr_multiplier_sl), 2
                )
                ...
                # Calculate take profit using ATR ratio
                take_profit_price = round(
                    current_price + (atr_value * atr_multiplier_tp), 2
                )
                ...
                if risk_per_share <= 0:
                    logger.warning(
                        f"Invalid risk_per_share for {symbol}: {risk_per_share}. Using default 1 share."
                    )
                    quantity = 1
                else:
                    ...
                    max_allowed_value = portfolio_equity * (
                    ...
                    if total_qty > max_allowed_qty:
                        ...
                        if quantity == 0:
                            logger.info(
                                f"Position size limit: existing {existing_qty} shares, max allowed {max_allowed_qty}. Skipping entry."
                            )
                            return AgentReport(
                                agent_id=self.agent_id,
                                status="success",
                            ...
                            logger.info(
                                f"Reduced quantity from {desired_quantity} to {quantity} due to position size limit (existing: {existing_qty})"
                            )
                    else:
            ...
            if proposals:
                flow(
                    "StrategyAgent",
                    "DONE",
                    f"{symbol} → BUY conf={confidence:.2f} qty={proposals[0].get('quantity', '?')} rules={len(entry_rules)}/{len(self.rule_strategy.entry_rules)}",
                )
    ...
    def _evaluate_strategies(
        self, indicators_report: AgentReport, news_report: AgentReport
    ) -> list[TradeProposal]:
        """
        ...
        if indicators_report and indicators_report.status == "success":
                ...
                if isinstance(ind_data, dict):
                    ...
                    try:
                        signal_enum = (
                            ...
                            if signal_str in SignalType.__members__
                            else SignalType(signal_str)
                        )
                    except:
                        signal_enum = SignalType.HOLD
                    timestamp = (
                        pd.Timestamp(ind_data.get("timestamp"))
                        if ind_data.get("timestamp")
            ...
            should_buy, buy_confidence, entry_rules = self.rule_strategy.evaluate_entry(
                indicators_snapshot
            )
            should_sell, exit_rules = self.rule_strategy.evaluate_exit(
                indicators_snapshot
            ...
            if should_buy and not should_sell:
                ...
                atr_value = (
                    atr_value.value if atr_value else (current_price * 0.02)
                )  # Default 2% of price if ATR not found

                target_price = round(current_price * 1.05, 2)  # Example: 5% above
                stop_loss_price = round(
                    current_price - (atr_value * 2), 2
                )  # Example: 2x ATR below
