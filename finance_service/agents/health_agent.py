...
class HealthAgent(Agent):
    ...
    def __init__(self, config_engine):
        ...
        try:
            self.drawdown_warning_pct = config_engine.get(
                "health", "drawdown_warning_pct", default=10.0
            )
            self.drawdown_critical_pct = config_engine.get(
                "health", "drawdown_critical_pct", default=20.0
            )
            self.check_interval_hours = config_engine.get(
                "health", "check_interval_hours", default=4
            )
            self.alert_cooldown_hours = config_engine.get(
                "health", "alert_cooldown_hours", default=4
            )
        ...
        try:
            ...
            self.remediation = RemediationHelper(
                config={}, venv_path=str(venv), workspace_path=str(workspace)
            )
    ...
    async def run(
        self, event_type: Optional[str] = None, payload: Optional[Dict[str, Any]] = None
    ) -> AgentReport:
        """
        Run health checks.
        ...
        if event_type == Events.SCHEDULE:
            ...
            return AgentReport(
                agent_id=self.agent_id, status="success", message="Daily summary sent"
            )
        ...
        return AgentReport(
            agent_id=self.agent_id, status="success", message="HealthAgent is running."
        )
    ...
    async def perform_health_check(self) -> AgentReport:
        ...
        if not self.portfolio_agent:
            return AgentReport(
                agent_id=self.agent_id, status="error", message="PortfolioAgent not set"
            )
        ...
        if portfolio_report.status != "success":
            logger.warning(
                f"Portfolio state retrieval failed: {portfolio_report.message}"
            )

            # Attempt auto-remediation for critical issues
            if self.remediation and (
                "500" in str(portfolio_report.message)
                or "async" in str(portfolio_report.message).lower()
            ):
                logger.info("Triggering auto-remediation for portfolio error")
                issue_detail = f"portfolio_error: {portfolio_report.message}"
                fix_result = await self.remediation.attempt_fix(
                    "portfolio_state_error", issue_detail
                )
                ...
                if fix_result.success:
                    ...
                    portfolio_report = (
                        await self.portfolio_agent.get_detailed_portfolio_state()
                    )
                    if portfolio_report.status == "success":
                        logger.info(
                            "Portfolio state fetch successful after remediation"
                        )
                    else:
                        logger.error(
                            f"Portfolio state still failing after remediation: {portfolio_report.message}"
                        )
                        return AgentReport(
                            agent_id=self.agent_id,
                            status="error",
                            message=f"Failed to get portfolio state even after remediation: {portfolio_report.message}",
                        )
                else:
                    return AgentReport(
                        agent_id=self.agent_id,
                        status="error",
                        message=f"Failed to get portfolio state and remediation failed: {fix_result.reason}",
                    )
            else:
                return AgentReport(
                    agent_id=self.agent_id,
                    status="error",
                    message=f"Failed to get portfolio state: {portfolio_report.message}",
                )
        ...
        if drawdown >= self.drawdown_critical_pct:
            alerts.append(
                f"⚠️ CRITICAL Drawdown: {drawdown:.2f}% (threshold: {self.drawdown_critical_pct}%)"
            )
            status = "critical"
        elif drawdown >= self.drawdown_warning_pct:
            alerts.append(
                f"⚠️ WARNING Drawdown: {drawdown:.2f}% (threshold: {self.drawdown_warning_pct}%)"
            )
            status = "warning"
        ...
        if total_equity < initial_cash * 0.9:
            alerts.append(
                f"📉 Equity below 90% of initial: ${total_equity:,.2f} vs ${initial_cash:,.2f}"
            )

        # yFinance / price fetch errors from portfolio agent
        if self.portfolio_agent and getattr(
            self.portfolio_agent, "last_price_fetch_error", None
        ):
            await self.send_error_alert(
                "Price Fetch Error", [self.portfolio_agent.last_price_fetch_error]
            )
        ...
        if alerts and self.last_alert_time:
            ...
            ).total_seconds() < self.alert_cooldown_hours * 3600:
                logger.info(
                    f"Health check alerts generated but in cooldown (last alert {self.last_alert_time})"
                )
        ...
        flow(
            "HealthAgent",
            "DONE",
            f"status={status} drawdown={drawdown:.1f}% equity=${total_equity:,.0f}",
        )
        return AgentReport(
            agent_id=self.agent_id,
            status="success",
    ...
    async def get_health_status(self) -> Dict[str, Any]:
        ...
        return (
            report.payload if report.status == "success" else {"error": report.message}
        )
    ...
    async def send_telegram_alert(self, alerts: list, metrics: Dict[str, Any]):
        ...
        if not self.telegram_agent or not self.telegram_agent.enabled:
            logger.warning(
                "TelegramAgent not configured or disabled, cannot send alerts"
            )
            return
        ...
        if not chat_id:
            logger.warning(
                "TelegramAgent has no chat_id configured, cannot send alerts"
            )
            return
    ...
    async def send_trade_notification(self, execution_payload: Dict[str, Any]):
        ...
        if not self.telegram_agent or not self.telegram_agent.enabled:
            logger.warning(
                "TelegramAgent not configured or disabled, cannot send trade notification"
            )
            return
        ...
        if not chat_id:
            logger.warning(
                "TelegramAgent has no chat_id configured, cannot send trade notification"
            )
            return
        ...
        if self.portfolio_agent:
            try:
                ...
                if portfolio_report.status == "success":
                    ...
                    portfolio_summary = (
                        f"Portfolio: ${equity:,.2f}, {positions} positions"
                    )
            except asyncio.TimeoutError:
                logger.warning(
                    "Portfolio state fetch timed out in trade notification; proceeding without portfolio context"
                )
            except Exception as e:
                logger.warning(
                    f"Could not get portfolio state for trade notification: {e}"
                )
        ...
        if action.upper() == "SELL":
            ...
            message += (
                f"• P&L: {pnl_sign}${realized_pnl:,.2f} ({pnl_sign}{pnl_pct:.2f}%)\n"
            )
    ...
    async def send_daily_summary(self):
        ...
        if not self.telegram_agent or not self.telegram_agent.enabled:
            logger.warning(
                "TelegramAgent not configured or disabled, cannot send daily summary"
            )
            return
        ...
        if not chat_id:
            logger.warning(
                "TelegramAgent has no chat_id configured, cannot send daily summary"
            )
            return

        try:
            portfolio_report = await asyncio.wait_for(
                self.portfolio_agent.get_detailed_portfolio_state(), timeout=3.0
            )
            if portfolio_report.status != "success":
                logger.error(
                    f"Failed to get portfolio state for daily summary: {portfolio_report.message}"
                )
                return
            ...
            if positions:
                ...
                for sym, pos in positions.items():
                    summary_lines.append(
                        f"  {sym}: {pos.quantity} @ ${pos.avg_cost:.2f}"
                    )
            ...
            if trades:
                ...
                for t in trades[-10:]:  # last 10 trades
                    summary_lines.append(
                        f"  {t.get('action')} {t.get('symbol')} x{t.get('quantity')} @ ${t.get('price'):,.2f}"
                    )
            ...
            logger.error(
                "Portfolio state fetch timed out for daily summary; cannot send summary"
            )
        except Exception as e:
