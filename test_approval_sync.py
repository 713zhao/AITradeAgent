...
def simulate_buy_proposal() -> Dict[str, Any]:
    print("\n" + "="*70)
    ...
    proposal = {
        ...
        "confidence": 0.65,  # Below 75% = requires approval
...
def simulate_risk_assessment(proposal: Dict[str, Any]) -> Dict[str, Any]:
    print("\n" + "="*70)
...
def send_telegram_approval_request(proposal: Dict[str, Any], risk: Dict[str, Any]) -> tuple:
    print("\n" + "="*70)
    ...
    try:
        ...
        task_id = f"TEST_BUY_{proposal['symbol']}_{int(time.time())}"
        ...
        # Send the approval request (synchronous)
        sent, msg = approval_gate.send_approval_request(task_id, proposal_summary, details)
        
        if sent:
            print(f"✅ Request sent to Telegram: {msg}")
            print(f"\n⏳ Waiting for response... (press Ctrl+C to cancel)")
            
            # Wait for response (synchronous)
            approved, response_msg = approval_gate.wait_for_response(task_id)
...
def execute_trade(proposal: Dict[str, Any]) -> Dict[str, Any]:
    print("\n" + "="*70)
    print("✅ TRADE EXECUTION")
    print("="*70)
    
    execution_result = {
        "trade_id": f"EXEC_{proposal['symbol']}_{int(time.time())}",
        "symbol": proposal['symbol'],
...
def main():
    ...
    try:
        # Step 1: Generate BUY proposal
        proposal = simulate_buy_proposal()
        
        # Step 2: Risk assessment
        risk = simulate_risk_assessment(proposal)
        ...
        # Step 3: Send Telegram approval request with buttons
        approved, message = send_telegram_approval_request(proposal, risk)
